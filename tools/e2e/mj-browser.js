...
const HTA = [
  ...
  '/* ══ 驱动到「轮到自己」的确定性辅助（断言口径：只保证前置状态，不放松断言）══════════',
  '   症状：ready_for_hint / hint_draw_fresh / 端到端（摸牌→点金框）会随机红，',
  '   实测值形如 over/0/14 —— 牌局在驱动到人类回合之前自己结束了，辅助函数拿不到人类回合。',
  '   旧写法（同步 for 循环 + 兜底 setHand）默认「牌局还活着」，没有任何终态分支，于是必红：',
  '     ① 摆的 13 张正好是听牌型（123456789万 + 1234条 单吊 4条），自己摸到 4条 时',
  '        Engine.turn 内部直接 settle() 自摸 → phase="over"，手牌停在 14 而 cur 仍是 0',
  '        → 断言读到的实测值就是 over/0/14（这一条正是历史日志里最常见的那次红）；',
  '     ② 牌墙摸完 → Engine.turn 开头的 drawGame() → phase="over"（cur 仍 0，手牌停在 13）；',
  '     ③ 本局在进辅助函数之前就已结束（AI 自摸 / 流局 → showResult → AUTO_FINISH_MS →',
  '        finishGame() 把 G.finished 置真）→ dbgAct 第一行就 return false，再怎么点都不会动。',
  '   新逻辑（状态机：轮询 → 终态判定 → 重开重试，有界 3 局）：',
  '     每轮读一次 state()：',
  '       · 终态①「轮到自己 + 手牌 %3==2」→ 成功返回（断言照旧必须真在人类回合）',
  '       · 终态②「finished / phase==="over"」→ 自动重开一局（dispose+start）+ 重新摆牌重试',
  '       · 窗口是自己的 claim/rob → pass；轮到自己但只有 13 张 → 真摸一张，回到轮询头部',
  '       · 别家的回合 → Mahjong.debug.step() 走一拍（与 pump() 同一条路径），回到轮询头部',
  '     断言一条没放松：仍然是「必须真在人类回合拿到提示 / 点了金框那张牌 / 打完仍听」。 */',
  'var DRIVE = { games: 0, polls: 0, restarts: 0, last: "" };',
  'function mjStOver(st) { return !st || st.finished === true || st.phase === "over"; }',
  'function mjStMine(st) { return !!(st && !mjStOver(st) && st.phase === "turn" && st.cur === 0 && st.handCount % 3 === 2); }',
  'function mjRestartGame() {',
  '  Mahjong.dispose();',
  '  var ok = Mahjong.start(document.getElementById("mj"), { onFinish: function (r) { FIN = r; } });',
  '  try { window.resizeTo(1300, 940); } catch (e) {}',
  '  DRIVE.restarts++;',
  '  return ok === true;',
  '}',
  '/* hand：要摆的手牌；wantDraw=true 表示「摆 13 张 + 真的自己摸一张」，false 表示「直接摆 14 张」 */',
  'function mjDriveToHumanTurn(hand, wantDraw, maxGames) {',
  '  var g, i, st, r, drew = false;',
  '  maxGames = maxGames || 3;',
  '  DRIVE.polls = 0; DRIVE.restarts = 0; DRIVE.last = "";',
  '  for (g = 0; g < maxGames; g++) {',
  '    DRIVE.games++;',
  '    if (g > 0) mjRestartGame();',
  '    drew = false;',
  '    if (Mahjong.debug.setHand(hand, [], null) !== true) { DRIVE.last += "setHand-false@" + g + "|"; continue; }',
  '    for (i = 0; i < 80; i++) {',
  '      DRIVE.polls++;',
  '      st = Mahjong.debug.state();',
  '      if (mjStMine(st)) return { ok: true, drew: drew, st: st, games: g + 1, polls: DRIVE.polls, restarts: DRIVE.restarts, trace: DRIVE.last };',
  '      if (mjStOver(st)) { DRIVE.last += "over@" + g + ":" + i + ":" + (st && st.phase) + "|"; break; }',
  ...
  '      if (st.phase === "turn" && st.cur === 0 && st.handCount % 3 === 1) {',
  '        if (wantDraw) { if (Mahjong.debug.act("draw") === true) drew = true; continue; }',
  '        break;',
  '      }',
  '      r = Mahjong.debug.step();',
  '      if (r === "off" || r === "over") { DRIVE.last += "step-" + r + "@" + g + ":" + i + "|"; break; }',
  '    }',
  '  }',
  '  return { ok: false, drew: drew, st: Mahjong.debug.state(), games: maxGames, polls: DRIVE.polls, restarts: DRIVE.restarts, trace: DRIVE.last };',
  '}',
  'function step2b() {',
'  var st, dr;',
'  try {',
'    /* 主链路开跑前确保轮到自己出牌：走统一驱动（轮询 → 终态判定 → 有界重开，最多 3 局）。',
'       原来这里是「同步 for 循环 + 兜底 setHand」—— 本局若已结束（流局 / 别家胡），',
'       循环直接 break、断言读到 over/x/y 必红；现在交给 mjDriveToHumanTurn。 */',
'    dr = mjDriveToHumanTurn(["1\\u4e07","1\\u4e07","2\\u4e07","3\\u4e07","4\\u4e07","5\\u4e07","6\\u4e07","7\\u4e07","8\\u4e07","9\\u4e07","1\\u6761","2\\u6761","3\\u6761","\\u4e2d"], false, 3);',
'    st = dr.st;',
'    info.driveChain = { ok: dr.ok, games: dr.games, polls: dr.polls, restarts: dr.restarts, trace: dr.trace };',
'    A("ready_for_chain", !!(dr.ok && st && st.phase === "turn" && st.cur === 0 && st.handCount % 3 === 2), st && (st.phase + "/" + st.cur + "/" + st.handCount));',
...
'    /* 13 张听牌型 + 自己真摸一张 → 「轮到自己 + 14 张」。',
'       旧写法直接同步 for 循环：这副牌是听 4条 的听牌型，真摸到 4条 会当场自摸结算',
'       （phase="over"、手牌 14、cur 仍 0）→ ready_for_hint 读到 over/0/14 必红。',
'       现在交给统一驱动：终态判定 + 有界重开（最多 3 局）。 */',
'    dr = mjDriveToHumanTurn(["1\\u4e07","2\\u4e07","3\\u4e07","4\\u4e07","5\\u4e07","6\\u4e07","7\\u4e07","8\\u4e07","9\\u4e07","1\\u6761","2\\u6761","3\\u6761","4\\u6761"], true, 3);',
'    st = dr.st;',
'    info.driveHint = { ok: dr.ok, drew: dr.drew, games: dr.games, polls: dr.polls, restarts: dr.restarts, trace: dr.trace };',
'    A("ready_for_hint", !!(dr.ok && st && st.phase === "turn" && st.cur === 0 && st.handCount % 3 === 2), st && (st.phase + "/" + st.cur + "/" + st.handCount));',
...
'  stepHint();',
  ...
  'function stepHint() {',
'  try {',
'    /* 同一类前置：把牌局确定性摆成「轮到自己 + 14 张」再读提示。',
'       旧写法只调一次 setHand：本局若已结束（G.finished 置真 / phase="over"），',
'       摆牌后 dbgAct 与 hintShouldCalc 都还是「已结算」，hintNow() 返回 null →',
'       整段跳过 → 十来条 hint_* 断言全部变成失败（历史日志里总数在 175~176 之间摆动就是这个）。',
'       现在走统一驱动：终态判定 + 有界重开（最多 3 局），断言口径不变。 */',
'    var sdrv = mjDriveToHumanTurn(["1\\u4e07","2\\u4e07","3\\u4e07","4\\u4e07","5\\u4e07","6\\u4e07","7\\u4e07","8\\u4e07","9\\u4e07","1\\u6761","2\\u6761","3\\u6761","4\\u6761","\\u4e2d"], false, 3);',
'    A("hint_setHand", sdrv.ok === true, sdrv.ok);',
'    info.driveHintSet = { ok: sdrv.ok, games: sdrv.games, polls: sdrv.polls, restarts: sdrv.restarts, trace: sdrv.trace };',
...
'         前置（摆 13 张 + 真的自己摸一张）交给统一驱动：轮询 → 终态判定 → 有界重开。',
'         旧写法只调一次 setHand + act("draw")：本局若已结束（牌墙摸完流局 / 别家胡后',
'         finishGame() 把 G.finished 置真），act("draw") 直接返回 false，',
'         drew 恒为 false → hint_draw_fresh 必红。断言表达式本身一个字没改。 */',
'      var tdrv = mjDriveToHumanTurn(["1\\u4e07","3\\u4e07","5\\u4e07","7\\u4e07","9\\u4e07","2\\u6761","4\\u6761","6\\u6761","8\\u6761","1\\u7b52","3\\u7b52","5\\u7b52","7\\u7b52"], true, 3);',
'      var drew = (tdrv.ok === true && tdrv.drew === true);',
...
'      info.driveTenpai = { ok: tdrv.ok, drew: tdrv.drew, games: tdrv.games, polls: tdrv.polls, restarts: tdrv.restarts, trace: tdrv.trace };',
...
'}',  '    info.hint = { discard: k.now && k.now.discard, idx: k.now && k.now.discardIdx, shanten: k.now && k.now.shanten, waits: k.now && k.now.waits, waitsLeft: k.now && k.now.waitsLeft, panel: k.brain.text.slice(0, 120), mark: { left: k.brain.markLeft, top: k.brain.markTop }, stats: k.stats };',
  ...
  '',
  '    /* ② 确保「轮到自己 + 14 张」：走统一驱动（轮询 → 终态判定 → 有界重开，最多 3 局）。',
'       旧写法是「同步 for 循环 + 兜底 setHand + 打一张」，本局若在语音段里已经打完',
'       （AI 自摸 / 流局 → finishGame()）就会卡在 over 上拿不到人类回合。 */',
'    var drv = mjDriveToHumanTurn(["1\\u4e07","1\\u4e07","2\\u4e07","3\\u4e07","4\\u4e07","5\\u4e07","6\\u4e07","7\\u4e07","8\\u4e07","9\\u4e07","1\\u6761","2\\u6761","3\\u6761","\\u4e2d"], false, 3);',
'    st = drv.st;',
'    info.driveVoice = { ok: drv.ok, games: drv.games, polls: drv.polls, restarts: drv.restarts, trace: drv.trace };',
'    A("voice_turn_ready", !!(drv.ok && st.phase === "turn" && st.cur === 0 && st.handCount % 3 === 2), st.phase + "/" + st.cur + "/" + st.handCount);',
  '',
  '    /* ③ 开关 / 映射 / say() */',
