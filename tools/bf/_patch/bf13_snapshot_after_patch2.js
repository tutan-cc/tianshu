/* ═══════════════════════════════════════════════════════════════════════════
   ...
   4) 「每个锅都配备一个空盘」：取消「3 个备菜盘限量」，9 列各配 1 个空盘（共 9 盘）。
      **盘上永久保鲜（bf-13 用户要求：「做好的早餐放旁边的盘子不要有倒计时，
...
(function (root) {
  ...
  /* ── 盘上永久保鲜（bf-13）的两个新常量 ────────────────────────────────────
     ...
     PLATE_NO_TIMER  = Infinity：盘上的「剩余秒数」哨兵 —— 既然没有倒计时，
                       它就不再是一个有限秒数，UI 层读它就知道「别画 N.Ns」，
                       而是画静态提示 PLATE_KEEP_TEXT。 */
  var PLATE_KEEP = true;
  ...
  /** 盘上停留会不会「超时变糊」—— bf-13 起**恒为 false**：盘上永久保鲜，不存在过期。
      ...
      —— 盘龄照旧记在 p.age 上，只是不再驱动任何判定。 */
  function plateBurntAt(ageSec) { return false; }
  /** 盘上这份还剩多少秒 —— bf-13 起**恒为 Infinity**：盘上没有倒计时。
      UI 层读它就是「不画倒计时」的信号（见 drawPlate / PLATE_KEEP_TEXT）。 */
  function plateLeftSec(ageSec) { return PLATE_NO_TIMER; }
  ...
  var BF_SFX_FILES = {
    tick:  ["tick.mp3"],
    /* 「拿到早餐」的欢呼（bf-11 用户拍板）：**4 条**变体，随机取、且不连着重复同一条
       （audio.pick() 里的 lastPick 保证不连重）。
       用户点名的 4 条（原话：「游戏里轮换『终于好啦』『开动啦』『哼，太慢了』这三个之前的语言。
       还有 v3 和 i45 共 5 个。」），顺序即拍板顺序：
         · happy_v1.mp3        「好耶！开动啦！」  → 用户说的**「开动啦」**
         · happy_v3.mp3        「哇哦！好香啊！」  → 用户说的 **v3**
         · happy_finally2.mp3  「太棒啦！终于好啦！」→ 用户说的**「终于好啦」**
         · happy_i45.mp3       「哇哦！」          → 用户说的 **i45**
       · slow.mp3「哼，太慢了」是**跑单**语音，走单独的 slow 通道，不是这个池子。
       ⚠ **本轮的一处替换（用户明确要求）**：用户先说「换成不含『呜呼』的版本」——
         原来这个位置是 happy_v2.mp3「**呜呼**！终于好啦！」（带他不喜欢的「呜呼」），
         现换成 happy_finally2.mp3「太棒啦！终于好啦！」（**不含「呜呼」**，实测 F0 +53.6%、
         谱质心 1189Hz、RMS 0.1050，omni 盲听两遍都听成「太棒了，终于好了」= 念对）。
         happy_v2.mp3 **文件仍留在盘上**（可能还要对比），但**已不在任何播放通道里**。
  ...
  /** 盘状态机（纯函数）：plated → 恒为落盘那一刻的档（bf-13 永久保鲜）。
      不再走 warm / cold，也不会因为「放久了」走到 burnt ——
      唯一还能返回 "burnt" 的情况是 p.state 本身就是 burnt（锅里糊了再端上盘）。 */
  function platePhaseOf(p, elapsed) {
    if (!p) return "none";
    if (p.state === "burnt") return "burnt";
    return heatTierOf(p.state, 0);          // 档位冻结在落盘那一刻（elapsed / age 不再参与）
  }
  ...
  /** 单击某一列的专属盘 → 出餐（要求 A3）。返回 { ok, kind, why, hint, ... }：
      ...
      · 此刻没人要这份  → no-want（不消耗，留在盘上 —— 永久保鲜，放多久都能上）
  ...
  function serveFromColumn(st, stationIdx) {
    ...
    if (ci < 0)
      return { ok:false, why:"no-want", hint:"现在没人要这份 —— 先放着（不会凉，放多久都能上）", food:p.food, station:stationIdx };
  ...
  function step(st, dt) {
    ...
    /* 专属盘：**永久保鲜**（bf-13）。这里只刷新「放了多久」这个记录值 p.age ——
       不降档、不倒数、不判过期、也不自动清盘：盘上那份放多久都在、放多久都是最高档。
       （锅内那条「忘出锅 → 糊」的路径在上面那一段，两条路径互不影响。） */
    for (i = 0; i < st.plates.length; i++) refreshPlate(st, st.plates[i]);
  ...
  function finish(st, win, reason) {
    ...
    /* bf-12 练手局：数值照常算（面板要显示服务 / 完美 / 糊），但 **bondDelta 归零**，
       impact / quota 也换成「练手局」口径 —— 页面层据此不落账，也不写 S.breakfastDay。 */
    var practice = !!st.cfg.practice;
    if (practice) delta = 0;
    st.result = {
      ...
      /* 兼容字段：prepPlates / serveWindow 保留原义；plateLife 语义已变 ——
      ...
      practice: practice,                    // 页面层据此跳过好感 / 每日一次 / 成就
      quote: quoteFor(role, st.win, quality, st.seq),
      impact: practice
        ? ("练手局 · 不影响好感（" + (target && target.name ? target.name : "对方") + " 的好感与今日次数都没变）")
        : impactOf(st.win, st.perfect, st.burnt, hot, target ? (target.name || "") : "对方"),
      quota: practice
        ? "练手局不算成绩 —— 想真送早餐就回侧栏「🍳 做份早餐」（好感 20~79 且今天没送过的角色才收得下）"
        : quotaOf(st.win, delta, target ? (target.name || "") : "对方"),
      reason: st.reason
    };
    return st.result;
  }
  ...
  function start(hostEl, opts) {
    ...
    /* 专属盘（每列一个，在锅的正上方）：空盘 = 浅色轮廓 + 列归属标签（「煎蛋盘 · 空」）；
       有食物 = 食物 + 档位 + **静态的「∞ 可一直放着」**（bf-13：盘上没有倒计时）；
       糊盘 = 红叉 + 「只能丢」（唯一的来源是「锅里糊了再端上盘」） */
    ...
    function drawPlate(b, p, s) {
      ...
      /* 盘上**没有倒计时**（bf-13 永久保鲜）：两行都是静态文案 ——
         不随盘龄变化、不红闪、不倒数。第一行 = 食材 + 档位（完美份恒为「热乎」），
         第二行 = 「∞ 可一直放着」这个明确承诺。 */
      g.fillStyle = "rgba(0,0,0,.55)"; roundRect(g, b.x + 2, b.y + b.h - 34, b.w - 4, 32, 8); g.fill();
      g.font = fontOf(FONT.micro, true);
      g.fillStyle = TIER_COLOR[tier] || PAL.ink;
      g.fillText((FOOD[p.food] || {}).n + "·" + (TIER_NAME[tier] || ""), cx, b.y + b.h - 24);
      g.font = fontOf(FONT.micro, false);
      g.fillStyle = "rgba(93,255,160,.85)";
      g.fillText(PLATE_KEEP_TEXT, cx, b.y + b.h - 10);
      g.textAlign = "left";
    ...
    function showResult() {
      ...
      /* bf-12 练手局：标题、顶部横幅、好感那一行三处都明确写「练手局 · 不影响好感」——
         用户要求「结算面板要明确标注」，不能只靠颜色暗示。 */
      var prac = !!r.practice;
      panel.innerHTML =
        '<h3>' + uiIconTag(win ? "check" : "cross", win ? "🍳" : "…") +
          (prac ? "练手局 · 不影响好感" : (win ? "送出热乎早餐" : "早餐烧坏了")) + '</h3>' +
        (prac ? '<div class="bf-imp" style="border:1px solid #ffd76e;color:#ffd76e;font-weight:700">🎯 练手局 · 不影响好感（不结算好感 / 不计每日一次 / 不写「今天已送」）</div>' : '') +
        ...
        '<div class="bf-grid">' +
          ...
          row(tgt.name + " 好感", prac
            ? '<b style="color:#ffd76e">不变（练手局）</b>'
            : '<b class="' + (r.bondDelta > 0 ? "up" : "down") + '">' + bondTxt + '</b>') +
        '</div>' +
        ...
        (prac ? '' : (win ? '' : '<div class="bf-imp dim">失败也有台阶：明天再来一次就行 —— 只要好感还在 20~79 之间。</div>'));
      var rowEl = el("div", "bf-row");
      var escHint = el("span", "bf-esc");
      escHint.textContent = "按 ESC 也可以退出（结算结果不会丢）";
      var go = mkBtn(prac ? "结束练手局 · 继续" : (win ? "收下早餐 · 继续" : "算了，明天再来 · 继续"), "primary bf-red");
      go.id = "bfGo";
    ...
    debug: {
      state: function () {
        ...
        return { running:st.running, over:st.over, elapsed:Math.round(st.elapsed * 100) / 100,
          ...
          prepped:st.prepped, autoPlate:!!st.cfg.autoPlate, practice:!!st.cfg.practice, plateCount:st.plates.length,
