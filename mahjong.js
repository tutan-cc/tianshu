...
(function (root) {
  ...
  function sh14(w, mc) {
    ...
    if (e !== undefined) return e;
    a = w.split("|");
    /* 已成牌 = -1：无副露看 evaluate；有副露时 stdShanten 必须**严格 < 0**（= 面子+将全齐）。
  ...
  var HINT_UKEIRE_MAX = 3;      // 「有效进张」逐张算的候选上限（兜底：极端手牌也不让一帧算爆）
  ...
  function drawMyHand(g) {
    ...
    for (i = 0; i < rects.length; i++) {
      ...
      if (i === markIdx && G.hintOn) {                // 智脑建议打出的那张：金色脉动边框（与 DOM 金框同一套定位规则）
  ...
  var G = {
    ...
    hover: -1, handRects: [], handRectsSig: "", wallSlots: null, noise: null, stat: null, tickTimer: 0, frames: 0,
  ...
  function calcHint() {
    ...
    var h = null;
    try {
      h = hintCalc({ hand: p.hand, melds: p.melds, drawn: p.drawn, seen: hintSeen(0), honors: G.E.honors });
      G.hintErr = "";
  ...
  /** 每次手牌变化 / 每帧兜底刷新一次（**同状态不重算**，重算判定与缓存签名共用 hintKeyOf）：返回当前提示 */
  function updateHint() {
    if (!G.E) return null;
    if (!G.hintOn || !hintShouldCalc()) { G.hint = null; G.hintIdx = -1; G.hintKey = ""; return null; }
    var p = G.E.P[0];
    var key = hintKeyOf(p.hand, p.melds, p.drawn, G.E.honors, hintSeen(0));
    if (key !== G.hintKey) { G.hintKey = key; G.hint = calcHint(); }
    /* 下标**永远**从「当前手牌 + 当前建议」现算 —— 不可能出现「文字说一张、高亮画另一张」 */
    G.hintIdx = G.hint ? indexOfTile(p.hand, G.hint.tile) : -1;
    return G.hint;
  }
  ...
  root.Mahjong = {
    ...
    test: {
      ...
      hintCalc: hintCalc, hintLines: hintLines, hintReason: hintReason, hintKeyOf: hintKeyOf,
