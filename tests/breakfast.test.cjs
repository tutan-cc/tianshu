...
test("音效：audio/bf 素材在磁盘上齐全，且规格统一（tick ≤120ms / 48kHz / 单声道）", () => {
  ...
  ["happy_alt1.mp3", "happy_alt4.mp3", "happy_alt5.mp3", "happy_alt6.mp3"].forEach(f => {
    assert.ok(fs.existsSync(at(f)), "对照件 " + f + " 还在盘上（bf-12 起归档在 _unused/，试听页要用）");
  ...
  assert.ok(fs.existsSync(at("happy_v2.mp3")), "happy_v2.mp3 文件仍留在盘上（bf-12 起归档在 _unused/，对比用）");
  ...
  const srcN56 = at("happy_n56.mp3");            // 源件也已归档到 _unused/，用 at() 找
  ...
  /* 欢呼候选 + 一条「太慢了」都要有人声时长（>0.4s），不是空文件。
